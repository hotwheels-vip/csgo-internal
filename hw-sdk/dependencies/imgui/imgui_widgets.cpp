...
void ImGui::LabelTextV( const char* label, const char* fmt, va_list args )
{
	...
	const ImRect total_bb( window->DC.CursorPos,
	                       window->DC.CursorPos + ImVec2( w + ( label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f ), style.FramePadding.y * 2 ) +
	                           label_size );
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
	...
	it_filled->second = ImClamp( it_filled->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( ( hovered ) ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled->second *= g.Style.Alpha;

	// Render
	RenderNavHighlight( bb, id );
	RenderFrame( bb.Min, bb.Max, ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, style.Alpha ), true, style.FrameRounding - 2 );

	PushStyleColor( ImGuiCol_Text, ImVec4( 1.f, 1.f, 1.f, it_filled->second ) );
...
void ImGui::Scrollbar( ImGuiAxis axis )
{
	...
	if ( axis == ImGuiAxis_X ) {
		...
		rounding_corners |=
			( ( window->Flags & ImGuiWindowFlags_NoTitleBar ) && !( window->Flags & ImGuiWindowFlags_MenuBar ) ) ? ImDrawCornerFlags_TopRight : 0;
	}
                   ...
                   const ImVec4& border_col )
{
	...
	if ( border_col.w > 0.0f ) {
		...
		window->DrawList->AddImageRounded( user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32( tint_col ), 2.f );
	}
}
...
bool ImGui::Checkbox( const char* label, bool* v, int x_pos )
{
	ImGuiWindow* window = GetCurrentWindow( );
	if ( window->SkipItems )
		return false;
	...
	it_filled->second = ImClamp( it_filled->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( ( hovered || *v ) ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled->second *= g.Style.Alpha;

	RenderFrame( check_bb.Min, check_bb.Max, ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha ), true, style.FrameRounding );
	...
	auto it_filled_frame = filled_animation.find( id );
	if ( it_filled_frame == filled_animation.end( ) ) {
		filled_animation.insert( { id, 0.f } );
		it_filled_frame = filled_animation.find( id );
	}
	it_filled_frame->second = ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( *v ? 2.f : -2.f ) ), 0.0f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;

	RenderFrame( check_bb.Min, check_bb.Max, color( 0 / 255.f, 0 / 255.f, 0 / 255.f ).get_u32( it_filled->second ), true, style.FrameRounding );

	RenderFrame( check_bb.Min, check_bb.Max, g_config.find< color >( HASH_BT( "m_menu_color" ) ).get_u32( it_filled_frame->second ), true,
	             style.FrameRounding );
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
	...
	const ImRect frame_bb( window->DC.CursorPos +
	                           ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f ),
	...
	if ( !( flags & ImGuiComboFlags_NoPreview ) )
		window->DrawList->AddRectFilled( frame_bb.Min, ImVec2( value_x2, frame_bb.Max.y ), ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, style.Alpha ),
		                                 style.FrameRounding,
		                                 ( flags & ImGuiComboFlags_NoArrowButton ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Left );
	if ( !( flags & ImGuiComboFlags_NoArrowButton ) ) {
		window->DrawList->AddRectFilled( ImVec2( value_x2, frame_bb.Min.y ), frame_bb.Max,
		                                 GetColorU32( hovered ? ImGuiCol_ControlBgHovered : ImGuiCol_ControlBgActive ), style.FrameRounding,
		                                 ( w <= arrow_size ) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right );
		if ( value_x2 + arrow_size - style.FramePadding.x <= frame_bb.Max.x )
			RenderArrow( window->DrawList, ImVec2( value_x2 + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y + 2 ),
			             ImColor( 1.f, 1.f, 1.f ), popup_open ? ImGuiDir_Up : ImGuiDir_Down, 0.5f );
	}
	...
	it_filled_frame->second = ImClamp(
		it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( hovered || g.ActiveId == id || popup_open ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;

	PushStyleColor( ImGuiCol_Text, ImVec4( 1.f, 1.f, 1.f, it_filled_frame->second ) );

	if ( preview_value != NULL && !( flags & ImGuiComboFlags_NoPreview ) )
		RenderTextClipped( ImVec2( frame_bb.Min.x + style.FramePadding.y, frame_bb.Min.y + style.FramePadding.y - 1.5f ),
		                   ImVec2( value_x2, frame_bb.Max.y - 1.5f ), preview_value, NULL, NULL, ImVec2( 0.0f, 0.0f ) );

	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x, total_bb.Min.y + style.FramePadding.y - 3 ), label );
	...
	if ( has_window_size_constraint ) {
		...
		// modified by qo0
		SetNextWindowSizeConstraints( ImVec2( w - ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f ), 0.0f ),
...
// FIXME: Move some of the code into SliderBehavior(). Current responsability is larger than what the equivalent DragBehaviorT<> does, we also do some
// rendering, etc.
template< typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE >
                            ...
                            const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb )
{
	...
	case ImGuiDataType_S8: {
		ImS32 v32 = ( ImS32 ) * ( ImS8* )p_v;
		bool r = SliderBehaviorT< ImS32, ImS32, float >( bb, id, ImGuiDataType_S32, &v32, *( const ImS8* )p_min, *( const ImS8* )p_max, format, power,
		                                                 flags, out_grab_bb );
		if ( r )
			*( ImS8* )p_v = ( ImS8 )v32;
		return r;
	...
	case ImGuiDataType_U8: {
		ImU32 v32 = ( ImU32 ) * ( ImU8* )p_v;
		bool r = SliderBehaviorT< ImU32, ImS32, float >( bb, id, ImGuiDataType_U32, &v32, *( const ImU8* )p_min, *( const ImU8* )p_max, format, power,
		                                                 flags, out_grab_bb );
		if ( r )
			*( ImU8* )p_v = ( ImU8 )v32;
		return r;
                          ...
                          float power )
{
	...
	// modified by qo0
	const ImRect frame_bb( ImVec2( window->DC.CursorPos.x - 1, window->DC.CursorPos.y ) +
	                           ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h - 1 : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f ),
	                       window->DC.CursorPos + ImVec2( w, ( label_size.x > 0.0f ? label_size.y : 0.0f ) + h ) );
	const ImRect total_bb( window->DC.CursorPos, frame_bb.Max - ImVec2( ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f ), 0.0f ) );
	...
	// Draw frame
	RenderNavHighlight( frame_bb, id );
	RenderFrame( frame_bb.Min, frame_bb.Max, ImColor( 20, 20, 20 ), true, g.Style.FrameRounding );
	...
	if ( grab_bb.Max.x > grab_bb.Min.x ) {
		window->DrawList->AddRectFilled( grab_bb.Min, grab_bb.Max, g_config.find< color >( HASH_BT( "m_menu_color" ) ).get_u32( ),
		                                 style.FrameRounding );
	...
	char value_buf[ 64 ];
	const char* value_buf_end = value_buf + DataTypeFormatString( value_buf, IM_ARRAYSIZE( value_buf ), data_type, p_data, format );
	RenderText( ImVec2( frame_bb.Max.x + 1.f - CalcTextSize( value_buf ).x - 1.0f, total_bb.Min.y + style.FramePadding.y - 1 ), value_buf,
	            value_buf_end );

	if ( label_size.x > 0.0f )
		RenderText( ImVec2( frame_bb.Min.x + 1.f, total_bb.Min.y + style.FramePadding.y - 1 ), label );
...
bool ImGui::SliderFloat( const char* label, float* v, float v_min, float v_max, const char* format, float power, int same_line_pos )
{
	SetCursorPosX( same_line_pos );
...
void ImGuiInputTextCallbackData::InsertChars( int pos, const char* new_text, const char* new_text_end )
{
	...
	if ( new_text_len + BufTextLen >= BufSize ) {
		...
		// Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 buffer, hence the midly similar code (until we remove the U16 buffer
		// alltogether!)
		ImGuiContext& g                 = *GImGui;
...
static bool InputTextFilterCharacter( unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data )
{
	...
	if ( flags & ( ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase |
                         ...
                         ImGuiInputTextCallback callback, void* callback_user_data )
{
	...
	// modified by qo0
	const ImVec2 frame_size = CalcItemSize( size_arg, CalcItemWidth( ) - ( label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f ),
	...
	if ( is_multiline ) {
		...
		draw_window->DC.NavLayerActiveMaskNext |=
			draw_window->DC.NavLayerCurrentMask; // This is to ensure that EndChild() will display a navigation highlight so we can "enter" into it.
	...
	const bool focus_requested_by_code =
		focus_requested && ( g.FocusRequestCurrWindow == window && g.FocusRequestCurrCounterRegular == window->DC.FocusCounterRegular );
	const bool focus_requested_by_tab = focus_requested && !focus_requested_by_code;

	const bool user_clicked = hovered && io.MouseClicked[ 0 ];
	const bool user_nav_input_start =
		( g.ActiveId != id ) && ( ( g.NavInputId == id ) || ( g.NavActivateId == id && g.NavInputSource == ImGuiInputSource_NavKeyboard ) );
	...
	if ( g.ActiveId != id && init_make_active ) {
		...
		if ( is_multiline )
			g.ActiveIdUsingKeyInputMask |=
				...
				( ( ImU64 )1 << ImGuiKey_PageDown ); // FIXME-NAV: Page up/down actually not supported yet by widget, but claim them ahead.
	...
	if ( g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id ) {
		...
		const bool is_osx          = io.ConfigMacOSXBehaviors;
		const bool is_shortcut_key = ( is_osx ? ( io.KeySuper && !io.KeyCtrl ) : ( io.KeyCtrl && !io.KeySuper ) ) && !io.KeyAlt &&
	...
	if ( g.ActiveId == id ) {
		...
		if ( apply_edit_back_to_user_buffer ) {
			...
			// User callback
			if ( ( flags & ( ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways ) ) !=
			     0 ) {
	...
	if ( !is_multiline ) {
		// modified by qo0
		RenderNavHighlight( frame_bb, id );
		window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha ),
		                                 style.FrameRounding );
		RenderFrame( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_ControlBg ), true, 4.f );
	}
	...
	auto it_filled_frame = hovered_animation.find( id );
	if ( it_filled_frame == hovered_animation.end( ) ) {
		hovered_animation.insert( { id, 0.f } );
		it_filled_frame = hovered_animation.find( id );
	}
	it_filled_frame->second =
		ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( hovered || g.ActiveId == id ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;

	PushStyleColor( ImGuiCol_Text, ImVec4( 1.f, 1.f, 1.f, it_filled_frame->second ) );
	if ( label_size.x > 0 )
		RenderText( ImVec2( frame_bb.Min.x, total_bb.Min.y + style.FramePadding.y - 5 ), label );
	PopStyleColor( );
...
// With typical options: Left-click on colored square to open color picker. Right-click to open option menu. CTRL-Click over input fields to edit them
...
bool ImGui::ColorEdit4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags )
{
	...
	if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) ) {
		...
		if ( BeginPopup( "picker" ) ) {
			...
			ImGuiColorEditFlags picker_flags = ( flags_untouched & picker_flags_to_forward ) | ImGuiColorEditFlags_DisplayHSV |
...
// responsible for avoiding that. I spent a non reasonable amount of time trying to getting this right for ColorButton with
...
// reasonable to disable rounding alltogether.
void ImGui::RenderColorRectWithAlphaCheckerboard( ImVec2 p_min, ImVec2 p_max, ImU32 col, float grid_step, ImVec2 grid_off, float rounding,
	...
	if ( ( ( col & IM_COL32_A_MASK ) >> IM_COL32_A_SHIFT ) < 0xFF ) {
		...
		for ( float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++ ) {
			...
			for ( float x = p_min.x + grid_off.x + ( yi & 1 ) * grid_step; x < p_max.x; x += grid_step * 2.0f ) {
				...
				window->DrawList->AddRectFilled( ImVec2( x1, y1 ), ImVec2( x2, y2 ), col_bg2, rounding_corners_flags_cell ? rounding : 0.0f,
...
static void ColorPickerOptionsPopup2( ImGuiColorEditFlags flags, float* ref_col )
{
	...
	if ( allow_opt_picker ) {
		ImVec2 picker_size( g.FontSize * 8, ImMax( g.FontSize * 8 - ( ( g.FontSize + g.Style.FramePadding.y * 2.0f ) + g.Style.ItemInnerSpacing.x ),
...
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col )
{
	...
	float sv_picker_size = 152; // ImMax( bars_width * 1, CalcItemWidth( ) - ( alpha_bar ? 2 : 1 ) * ( bars_width + style.ItemInnerSpacing.x ) ); //
	...
	if ( flags & ImGuiColorEditFlags_PickerHueBar ) {
		...
		draw_list->AddRect( ImVec2( bar0_pos_x - 1, picker_pos.y - 0 ), ImVec2( bar0_pos_x + bars_width, picker_pos.y + sv_picker_size + 0 ),
		                    IM_COL32( 0, 0, 0, 255 ) );
	}

	float sv_cursor_rad = 3;
	draw_list->AddRectFilled( sv_cursor_pos - ImVec2( 2, 2 ), sv_cursor_pos + ImVec2( 2, 2 ), IM_COL32( 0, 0, 0, 255 ), style.FrameRounding );
	draw_list->AddRectFilled( sv_cursor_pos - ImVec2( 1, 1 ), sv_cursor_pos + ImVec2( 1, 1 ), col32_no_alpha, style.FrameRounding );
	...
	if ( alpha_bar ) {
		...
		draw_list->AddRectFilled( ImVec2( bar1_pos_x + 0, bar1_line_y + 1 ), ImVec2( bar1_pos_x + bars_width - 1, bar1_line_y + 3 ),
		                          IM_COL32( col[ 0 ], col[ 1 ], col[ 2 ], col[ 3 ] ) );
	}
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size )
{
	...
	if ( size.y == 0.0f )
		size.y = default_size;
	const ImRect bb( ImVec2( window->DC.CursorPos.x - 8, window->DC.CursorPos.y ), window->DC.CursorPos + size );
	...
	float rounding  = g.Style.FrameRounding - 1;
	ImRect bb_inner = bb;
	float off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This offset seemed like
	                    // a good middle ground to reduce those artifacts.
	bb_inner.Expand( off );
	if ( ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) && col_rgb.w < 1.0f ) {
		float mid_x = IM_ROUND( ( bb_inner.Min.x + bb_inner.Max.x ) * 0.5f );
		RenderColorRectWithAlphaCheckerboard( ImVec2( bb_inner.Min.x + grid_step, bb_inner.Min.y ), bb_inner.Max, GetColorU32( col_rgb ), grid_step,
	...
	window->DrawList->AddRect( bb.Min, bb.Max, ImColor( 20 / 255.f, 20 / 255.f, 20 / 255.f, g.Style.Alpha ),
	...
	if ( !( flags & ImGuiColorEditFlags_NoTooltip ) && hovered )
		ColorTooltip( desc_id, &col.x,
		              flags & ( ImGuiColorEditFlags__InputMask | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview |
...
bool ImGui::TreeNodeBehavior( ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end )
{
	...
	if ( flags & ImGuiTreeNodeFlags_OpenOnDoubleClick )
		button_flags |=
			ImGuiButtonFlags_PressedOnDoubleClick | ( ( flags & ImGuiTreeNodeFlags_OpenOnArrow ) ? ImGuiButtonFlags_PressedOnClickRelease : 0 );
	else
	...
	if ( !is_leaf ) {
		if ( pressed ) {
			...
			if ( flags & ImGuiTreeNodeFlags_OpenOnArrow )
				toggled |= ( g.IO.MousePos.x >= arrow_hit_x1 && g.IO.MousePos.x < arrow_hit_x2 ) &&
	...
	if ( display_frame ) {
		...
		if ( g.LogEnabled ) {
			// NB: '##' is normally used to hide text (as a library-wide feature), so we need to specify the text range to make sure the ## aren't
			// stripped out here.
...
bool ImGui::CollapsingHeader( const char* label, bool* p_open, ImGuiTreeNodeFlags flags )
{
	...
	flags |=
		ImGuiTreeNodeFlags_CollapsingHeader | ( p_open ? ImGuiTreeNodeFlags_AllowItemOverlap | ImGuiTreeNodeFlags_ClipLabelForTrailingButton : 0 );
...
bool ImGui::Selectable( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg )
{
	...
	it_filled_frame->second =
		ImClamp( it_filled_frame->second + ( 2.35f * ImGui::GetIO( ).DeltaTime * ( selected || hovered ? 2.f : -2.f ) ), 0.5f, 1.f );
	it_filled_frame->second *= g.Style.Alpha;
...
bool ImGui::ListBoxHeader( const char* label, int items_count, int height_in_items )
{
	...
	// We don't add this extra bit if items_count <= height_in_items. It is slightly dodgy, because it means a dynamic list of items will make the
	// widget resize occasionally when it crosses that size. I am expecting that someone will come and complain about this behavior in a remote
	// future, then we can advise on a better solution.
	if ( height_in_items < 0 )
		height_in_items = ImMin( items_count, 7 );
	const ImGuiStyle& style = GetStyle( );
	float height_in_items_f = ( height_in_items < items_count ) ? ( height_in_items + 0.25f ) : ( height_in_items + 0.00f );

	// We include ItemSpacing.y so that a list sized for the exact number of items doesn't make a scrollbar appears. We could also enforce that by
                     ...
                     int height_in_items )
{
	...
	// Assume all items have even height (= 1 line of text). If you need items of different or variable sizes you can create a custom version of
	// ListBox() in your code without using the clipper.
	ImGuiContext& g    = *GImGui;
	bool value_changed = false;
	ImGuiListClipper clipper( items_count, GetTextLineHeightWithSpacing( ) ); // We know exactly our line height here so we pass it as a minor
                    ...
                    int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 frame_size )
{
	...
	if ( values_count >= values_count_min ) {
		...
		float histogram_zero_line_t =
			( scale_min * scale_max < 0.0f ) ? ( -scale_min * inv_scale ) : ( scale_min < 0.0f ? 0.0f : 1.0f ); // Where does the zero line stands
...
void ImGui::EndMenuBar( )
{
	...
	if ( NavMoveRequestButNoResultYet( ) && ( g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right ) &&
		...
		if ( nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal &&
			...
			// window (probably not worth the hassle/cost)
...
bool ImGui::BeginMenu( const char* label, bool enabled )
{
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
		...
		pressed = Selectable( label, menu_is_open,
		                      ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_PressedOnClick | ImGuiSelectableFlags_DontClosePopups |
		...
		RenderArrow( window->DrawList, pos + ImVec2( window->DC.MenuColumns.Pos[ 2 ] + extra_w + g.FontSize * 0.30f, 0.0f ), text_col,
		             ImGuiDir_Right );
	}
	...
	if ( window->DC.LayoutType == ImGuiLayoutType_Vertical ) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if ( g.HoveredWindow == window && child_menu_window != NULL && !( window->Flags & ImGuiWindowFlags_MenuBar ) ) {
			...
			// GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_within_opened_triangle ? IM_COL32(0,128,0,128) : IM_COL32(128,0,0,128));
			// // [DEBUG]
		}
	...
	if ( menu_is_open ) {
		// Sub-menus are ChildWindow so that mouse can be hovering across them (otherwise top-most popup menu would steal focus and not allow hovering
		// on parent menu)
...
bool ImGui::MenuItem( const char* label, const char* shortcut, bool selected, bool enabled )
{
	...
	ImGuiSelectableFlags flags =
		ImGuiSelectableFlags_PressedOnRelease | ImGuiSelectableFlags_SetNavIdOnHover | ( enabled ? 0 : ImGuiSelectableFlags_Disabled );
	bool pressed;
	if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
		...
		if ( selected )
			RenderCheckMark( pos + ImVec2( window->DC.MenuColumns.Pos[ 2 ] + extra_w + g.FontSize * 0.40f, g.FontSize * 0.134f * 0.5f ),
...
bool ImGui::TabItemEx( ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
	...
	// If the user called us with *p_open == false, we early out and don't render. We make a dummy call to ItemAdd() so that attempts to use a
...
bool ImGui::TabItemLabelAndCloseButton( ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label,
