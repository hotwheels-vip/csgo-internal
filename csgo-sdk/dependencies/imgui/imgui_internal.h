...
enum ImGuiItemFlags_ {
	...
	ImGuiItemFlags_Inputable = 1 << 10, // false     // [WIP] Auto-activate input mode when tab focused. Currently only used and supported by a few
...
enum ImGuiButtonFlagsPrivate_ {
	...
	ImGuiButtonFlags_NoNavFocus =
		1 << 18, // don't override navigation focus when activated (FIXME: this is essentially used everytime an item uses ImGuiItemFlags_NoNav, but
	             // because legacy specs don't requires LastItemData to be set ButtonBehavior(), we can't poll g.LastItemData.InFlags)
...
enum ImGuiSelectableFlagsPrivate_ {
	...
	ImGuiSelectableFlags_NoSetKeyOwner = 1 << 28, // Don't set key/input owner on the initial click (note: mouse buttons are keys! often, the key in
	                                              // question will be ImGuiKey_MouseLeft!)
};
...
enum ImGuiInputFlags_ {
	...
	ImGuiInputFlags_LockThisFrame =
		1 << 6, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for polling). Cleared at end of frame.
	            // This is useful to make input-owner-aware code steal keys from non-input-owner-aware code.
	ImGuiInputFlags_LockUntilRelease = 1 << 7, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT accepted for
	...
	ImGuiInputFlags_RouteGlobalLow = 1 << 9, // Register route globally (lowest priority: unless a focused window or active item registered the route)
	                                         // -> recommended Global priority.
	...
	ImGuiInputFlags_RouteUnlessBgFocused = 1 << 13, // Global routes will not be applied if underlying background/void is focused (== no Dear ImGui
	...
	ImGuiInputFlags_SupportedByShortcut =
		ImGuiInputFlags_Repeat | ImGuiInputFlags_RepeatRateMask_ | ImGuiInputFlags_RouteMask_ | ImGuiInputFlags_RouteExtraMask_,
...
struct ImGuiStackLevelInfo {
	...
	char Desc[ 57 ]; // Arbitrarily sized buffer to hold a result (FIXME: could replace Results[] with a chunk stream?) FIXME: Now that we added
	                 // CTRL+C this should be fixed.
...
struct ImGuiContext {
	...
	bool ActiveIdUsingAllKeyboardKeys; // Active widget will want to read all keyboard keys inputs. (FIXME: This is a shortcut for not taking
	                                   // ownership of 100+ keys but perhaps best to not have the inconsistency)
#	ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
	ImU32 ActiveIdUsingNavInputMask; // If you used this. Since (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1 <<
	                                 // ImGuiNavInput_Cancel);' becomes 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/or
...
#	endif
	...
	ImGuiID NavFocusScopeId; // Identify a selection scope (selection code often wants to "clear other items" when landing on an item of the selection
	                         // set)
	ImGuiID NavActivateId;   // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) || IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also
...
// Per-instance data that needs preserving across frames (seemingly most others do not need to be preserved aside from debug needs, does that needs
// they could be moved to ImGuiTableTempData ?)
...
namespace ImGui
{
	...
	IMGUI_API bool BeginChildEx( const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags,
	                             const std::vector< const char* > subtab_names, static int* subtab_number = nullptr, bool show_text = true );
	...
	IsKeyPressed( ImGuiKey key, ImGuiID owner_id,
	              ImGuiInputFlags flags = 0 ); // Important: when transitioning from old to new IsKeyPressed(): old API has "bool repeat = true", so
	                                           // would default to repeat. New API requiress explicit ImGuiInputFlags_Repeat.
	...
	// - When using one of the routing flags (e.g. ImGuiInputFlags_RouteFocused): routes requested ahead of time given a chord (key + modifiers) and a
	// routing policy.
	...
	IMGUI_API bool ButtonEx( const char* label, const ImVec2& size_arg = ImVec2( 0, 0 ), ImGuiButtonFlags flags = 0, bool underneath_checkbox = false );
	...
	IMGUI_API bool TreeNodeUpdateNextOpen(
		ImGuiID id, ImGuiTreeNodeFlags flags ); // Return open state. Consume previous SetNextItemOpen() data, if any. May return true when logging.
