...
constexpr unsigned int crc_table[ NUM_BYTES ] = {
...
void crc32::init( unsigned int* crc )
{
	*crc = CRC32_INIT_VALUE;
}

void crc32::end( unsigned int* crc )
{
	*crc ^= CRC32_XOR_VALUE;
}

unsigned int crc32::get_table_entry( unsigned int nSlot )
{
	return crc_table[ ( unsigned char )nSlot ];
}

void crc32::process_buffer( unsigned int* crc, const void* buffer_pointer, int buffer )
{
	unsigned int ucrc     = *crc;
	unsigned char* pb = ( unsigned char* )buffer_pointer;
	unsigned int ufront;
	int nMain;
...
JUST_A_FEW:
	...
	switch ( buffer ) {
	case 7:
		ucrc = crc_table[ *pb++ ^ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
	...
	case 4:
		ucrc ^= LITTLE_LONG( *( unsigned int* )pb );
		ucrc   = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc   = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc   = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc   = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		*crc = ucrc;
	...
	//
	// The low-order two bits of pb and buffer in total control the
	// upfront work.
	//
	ufront = ( ( unsigned int )pb ) & 3;
	buffer -= ufront;
	switch ( ufront ) {
	case 3:
		ucrc = crc_table[ *pb++ ^ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
	...
	while ( nMain-- ) {
		...
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc ^= LITTLE_LONG( *( unsigned int* )( pb + 4 ) );
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
		ucrc = crc_table[ ( unsigned char )ucrc ] ^ ( ucrc >> 8 );
