...
c_base_entity* n_aimbot::impl_t::find_closest_player( )
{
	...
	g_entity_cache.enumerate( e_enumeration_type::type_players, [ & ]( c_base_entity* entity ) {
		...
		const auto angle = g_math.calculate_angle( eye_position, entity->get_hitbox_position( hitgroup_head ) );
...
void n_aimbot::impl_t::on_create_move_post( )
{
	...
	c_vector forward_to_head = g_ctx.m_local->get_eye_position( false ) - entity->get_hitbox_position( hitgroup_head );
	c_angle angles_to_head;

	g_math.vector_angles( forward_to_head, angles_to_head );

	// if valid record, recalculate to matrix hitbox pos
	if ( g_ctx.m_record ) {
		forward_to_head = g_ctx.m_local->get_eye_position( false ) - entity->get_hitbox_position( hitgroup_head, g_ctx.m_record->m_matrix );
		g_math.vector_angles( forward_to_head, angles_to_head );
	}
