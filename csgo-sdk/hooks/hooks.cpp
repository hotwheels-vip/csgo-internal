...
bool n_hooks::impl_t::on_attach( )
{
	...
	initialise_hook( this->m_on_add_entity,
	                 reinterpret_cast< void* >( g_modules[ HASH_CT( "client.dll" ) ].find_pattern( "55 8B EC 51 8B 45 0C 53 56 8B F1 57" ) ),
	                 &n_detoured_functions::on_add_entity, "IClientEntityList::OnAddEntity()" );

	initialise_hook(
		this->m_on_remove_entity,
		reinterpret_cast< void* >( g_modules[ HASH_CT( "client.dll" ) ].find_pattern( "55 8B EC 51 8B 45 0C 53 8B D9 56 57 83 F8 FF 75 07" ) ),
		&n_detoured_functions::on_remove_entity, "IClientEntityList::OnRemoveEntity()" );

	initialise_hook( this->m_level_init_pre_entity, g_virtual.get( g_interfaces.m_base_client, 5 ), &n_detoured_functions::level_init_pre_entity,
	                 "CHLClient::LevelInitPreEntity()" );

	initialise_hook( this->m_level_shutdown, g_virtual.get( g_interfaces.m_base_client, 7 ), &n_detoured_functions::level_shutdown,
