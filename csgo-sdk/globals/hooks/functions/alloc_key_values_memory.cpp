...
void* __fastcall n_detoured_functions::alloc_key_values_memory( void* ecx, void* edx, int size )
{
	...
	static const auto key_values_alloc_engine =
		g_relative.get( reinterpret_cast< unsigned int >(
			g_modules[ HASH_CT( "engine.dll" ) ].find_pattern( ( "E8 ? ? ? ? 83 C4 08 84 C0 75 10 FF 75 0C" ) ) + 0x1 ) ) +
		0x4A;

	static const auto key_values_alloc_client =
		g_relative.get(
			reinterpret_cast< unsigned int >( g_modules[ HASH_CT( "client.dll" ) ].find_pattern( ( "E8 ? ? ? ? 83 C4 08 84 C0 75 10" ) ) + 0x1 ) ) +
		0x3E;
