...
int __cdecl hooks::vsnprintf( char* dest, int text_length, const char* fmt, ... )
{
	int v4 = { }; // ecx
	...
	if ( strstr( fmt, xs( "fps:" ) ) ) {
		/*fmt = g_interfaces.m_engine->is_in_game( ) ? xs( "fps: %5i / %4.1f var / %i ms" ) : xs( "fps: %5i / var: %4.1f ms / (v%u%s)" );*/
		fmt = xs( "fps: %5i / var: %4.1f ms / (v%u%s)" );
