...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderFrame(bb.Min - ImVec2(2.f, 2.f), bb.Max + ImVec2(2.f, 2.f), ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f), true, g.Style.FrameRounding);
    RenderFrame(bb.Min - ImVec2(2.f, 2.f), bb.Max + ImVec2(2.f, 2.f), ImColor(Accent[0], Accent[1], Accent[2], 1.f * frame_animation.AnimationData->second), true, g.Style.FrameRounding);
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4(text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * text_animation.AnimationData->second));

    RenderTextClipped(bb.Min + style.FramePadding + ImVec2(3.f, 0.f), bb.Max - style.FramePadding + ImVec2(3.f, 0.f), label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    ScrollbarEx(ImRect(bb.Min + ImVec2(2.f, 0.f), bb.Max + ImVec2(2.f, 0.f)), id, axis, &scroll, (ImS64)size_avail, (ImS64)size_contents, rounding_corners);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor::Blend(ImColor(51 / 255.f, 51 / 255.f, 51 / 255.f, 1.f), ImColor(Accent[0], Accent[1], Accent[2]), scrollbar_animation.AnimationData->second), style.ScrollbarRounding);
...
// If you need to change padding with new ImageBILL utton() you can use PushStyleVar(ImGuiStyleVar_FramePadding, value), consistent with other Button functions.
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f) + 2.f, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    RenderFrame(check_bb.Min, check_bb.Max, ImColor(Accent[0], Accent[1], Accent[2], 1.f * frame_animation.AnimationData->second), true, style.FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2((label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f) - 3.f, label_size.x > 0.0f ? label_size.y : 0.0f), window->DC.CursorPos + ImVec2(w, (label_size.x > 0.0f ? label_size.y : 0.0f) + h));
    ...
    {
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(value_x2, frame_bb.Max.y), ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f), style.PopupRounding);
    ...
    else
    {
        ...
        // modified by qo0
        SetNextWindowSizeConstraints(ImVec2(w - (label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f) + 3.f, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items)));
    }
    ...
    PushClipRect(ImGui::GetWindowPos(), ImGui::GetWindowSize() + ImGui::GetWindowPos(), false)
        ;    ImGui::GetWindowDrawList()->AddRect(ImGui::GetWindowPos(), ImGui::GetWindowSize() + ImGui::GetWindowPos(), ImColor(50, 50, 50, 100), style.FrameRounding);
    PopClipRect();
...
bool ImGui::MultiCombo(const char* label, std::vector< bool >& values, const std::vector<const char*>& item_list, int items_count)
{
    ...
    if (buffer.empty())
        ...
        const char* wrap_pos =
            g.Font->CalcWordWrapPositionA(GetCurrentWindow()->FontWindowScale, &buffer[0], buffer.data() + buffer.length(),
...
bool ImGui::Keybind(const char* label, KeyBind_t* key_data, bool has_style)
{
    ...
    window->DrawList->AddText(frame_bb.Min, ImColor::Blend(ImColor(text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * 0.5f), ImColor(Accent[0], Accent[1], Accent[2]), text_animation.AnimationData->second), buf_display);
...
bool ImGui::CurveEditor(const char* label, Points_t* points, ImVec2 data[], int members)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(18.f, 0.f), window->DC.CursorPos + ImVec2(18.f, 0.f) + ImVec2(ImGui::GetContentRegionAvail().x - 29.f, 100.f));
    const ImRect total_bb(window->DC.CursorPos + ImVec2(18.f, 0.f), window->DC.CursorPos + ImVec2(18.f, 0.f) + ImVec2(ImGui::GetContentRegionAvail().x - 29.f, 100.f) + label_size + ImVec2(0.f, 5.f));
    ...
    // Draw the points
    const auto point_position_a = ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * points->PointA.x, frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) * (1.f + -(points->PointA.y)));
    const auto point_position_b = ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * points->PointB.x, frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) * (1.f + -(points->PointB.y)));

    const auto point_size = 10.f;

    // Fix point position clipping off of the curve editor
    const auto point_position_a_clamped = ImVec2(ImClamp(point_position_a.x, frame_bb.Min.x + point_size, frame_bb.Max.x - point_size), ImClamp(point_position_a.y, frame_bb.Min.y + point_size, frame_bb.Max.y - point_size));
    const auto point_position_b_clamped = ImVec2(ImClamp(point_position_b.x, frame_bb.Min.x + point_size, frame_bb.Max.x - point_size), ImClamp(point_position_b.y, frame_bb.Min.y + point_size, frame_bb.Max.y - point_size));
    ...
    const auto point_a_bb = ImRect(point_position_a_clamped - ImVec2(point_size, point_size), point_position_a_clamped + ImVec2(point_size, point_size));
    const auto point_b_bb = ImRect(point_position_b_clamped - ImVec2(point_size, point_size), point_position_b_clamped + ImVec2(point_size, point_size));
    ...
    for (int i = 0; i < members; i++)
    {
        ...
        const auto point_position = ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * pos.x, frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) * (1.f + -pos.y));
        ...
        if (i > 0)
        {
            ...
            const auto prev_point_position = ImVec2(frame_bb.Min.x + (frame_bb.Max.x - frame_bb.Min.x) * prev_pos.x, frame_bb.Min.y + (frame_bb.Max.y - frame_bb.Min.y) * (1.f + -prev_pos.y));
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(200 / 255.f, 255 / 255.f, 80 / 255.f, 1.f * frame_animation.AnimationData->second), true, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + old_value_data->second, frame_bb.Max.y), ImColor(Accent[0], Accent[1], Accent[2], 1.f), old_value_data->second > 1.f ? g.Style.FrameRounding : 0.f);
    ...
    PushStyleColor(ImGuiCol_Text, ImVec4(text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * text_animation.AnimationData->second));
    RenderText(ImVec2(frame_bb.Max.x - CalcTextSize(value_buf).x + 1.f, total_bb.Min.y + style.FramePadding.y - 3), value_buf,
        value_buf_end);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y + 4.f * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    text_animation.Update(hovered || g.ActiveId
        == id ? 4.f : -4.f, 1.f, 0.5f, 1.f);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
        ...
        draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, bar0_line_y - 10), ImVec2(bar0_pos_x + bars_width, bar0_line_y), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.6f), ImColor(0.f, 0.f, 0.f, 0.6f));
        draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, bar0_line_y + 10), ImVec2(bar0_pos_x + bars_width, bar0_line_y), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.6f), ImColor(0.f, 0.f, 0.f, 0.6f));
    ...
    draw_list->AddRect(sv_cursor_pos - ImVec2(1.f, 1.f), sv_cursor_pos + ImVec2(2.f, 2.f) + ImVec2(1.f, 1.f), ImColor(25 / 255.f, 25 / 255.f, 25 / 255.f, 1.f));
    // draw_list->AddRect(sv_cursor_pos - ImVec2(2.f, 2.f), sv_cursor_pos + ImVec2(2.f, 2.f) + ImVec2(2.f, 2.f), ImColor(10 / 255.f, 10 / 255.f, 10 / 255.f, 1.f));
    ...
    if (alpha_bar) {
        ...
        draw_list->AddRectFilledMultiColor(ImVec2(bar1_pos_x, bar1_line_y - 10), ImVec2(bar1_pos_x + bars_width, bar1_line_y), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.6f), ImColor(0.f, 0.f, 0.f, 0.6f));
        draw_list->AddRectFilledMultiColor(ImVec2(bar1_pos_x, bar1_line_y + 10), ImVec2(bar1_pos_x + bars_width, bar1_line_y), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.f), ImColor(0.f, 0.f, 0.f, 0.6f), ImColor(0.f, 0.f, 0.f, 0.6f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec4 blended_color = ImColor::Blend(ImColor(text_color.Value.x, text_color.Value.y, text_color.Value.z, text_color.Value.w * hovered_text_animation.AnimationData->second), ImColor(Accent[0], Accent[1], Accent[2]), selected_text_animation.AnimationData->second);
