...
void prediction_t::begin( c_user_cmd* cmd )
{
	...
	*prediction.m_prediction_random_seed = globals.m_cmd->m_random_seed;
	*prediction.m_prediction_player      = globals.m_local;
	...
	memory.m_globals->m_current_time = globals.m_local->tick_base( ) * memory.m_globals->m_interval_per_tick;
	...
	//
	// if ( int* next_think_tick = globals.m_local->next_think_tick( ); *next_think_tick > 0 && *next_think_tick <= prediction.get_tick_base( ) ) {
	//	*next_think_tick = -1;
	//
	...
	interfaces.m_prediction->setup_move( globals.m_local, globals.m_cmd, memory.m_move_helper, &move_data );

	interfaces.m_game_movement->process_movement( globals.m_local, &move_data );

	interfaces.m_prediction->finish_move( globals.m_local, globals.m_cmd, &move_data );
