...
bool interfaces_t::on_attach( )
{
	...
	if ( ( this->m_convar = this->find< c_convar >( memory.m_modules[ e_module_names::vstdlib ].m_value, xs( "VEngineCvar" ) ) ) == nullptr )
		return false;

	if ( ( this->m_game_movement = this->find< c_game_movement >( memory.m_modules[ e_module_names::client ].m_value, xs( "GameMovement" ) ) ) == nullptr )
		return false;

	if ( ( this->m_prediction = this->find< c_prediction >( memory.m_modules[ e_module_names::client ].m_value, xs( "VClientPrediction" ) ) ) == nullptr )
		return false;

	if ( ( this->m_mdl_cache = this->find< c_mdl_cache >( memory.m_modules[ e_module_names::datacache ].m_value, xs( "MDLCache" ) ) ) == nullptr )
		return false;
	...
	const HSteamUser steam_user = reinterpret_cast< std::add_pointer_t< HSteamUser( ) > >(
		utilities.get_export_address( memory.m_modules[ e_module_names::steam_api ].m_value, xs( "SteamAPI_GetHSteamUser" ) ) )( );
	const HSteamPipe steam_pipe = reinterpret_cast< std::add_pointer_t< HSteamPipe( ) > >(
		utilities.get_export_address( memory.m_modules[ e_module_names::steam_api ].m_value, xs( "SteamAPI_GetHSteamPipe" ) ) )( );

	SteamGameCoordinator =
		static_cast< ISteamGameCoordinator* >( SteamClient->GetISteamGenericInterface( steam_user, steam_pipe, xs( "SteamGameCoordinator001" ) ) );
