...
{
	const auto m_vis_color   = GET_CONFIG_COLOR( variables.m_visuals.m_glow_vis_color );
	const auto m_invis_color = GET_CONFIG_COLOR( variables.m_visuals.m_glow_invis_color );

	c_vector head_position = player->get_bone_position( e_hitgroup::hitgroup_head );

	const bool can_see_player = local->is_visible( player, head_position );
	...
	return GET_CONFIG_BOOL( variables.m_visuals.m_glow_enable ) && player != local && player->team( ) != local->team( );
}
